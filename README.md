<h1 align="center">👋 Bonjour, je suis Rania Jrad</h1>

<p align="center">
  <strong>👩‍💻 Junior Full Stack Developer | 🎓 Élève ingénieure en informatique</strong>
</p>

<p align="center">
  <em>Développement Web • Développement Mobile • Génie Logiciel</em>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=raniajrad&label=Visiteurs%20du%20profil&color=8A2BE2&style=for-the-badge" alt="Visiteurs du profil" />
</p>

---

## 👩‍💻 À propos de moi

Je suis **Rania Jrad**, développeuse **Full Stack Junior** et **élève ingénieure en informatique**, diplômée en **Technologie de l'Informatique, spécialité Développement des Systèmes d'Information**.

Passionnée par le développement logiciel, je m'intéresse particulièrement à la conception et au développement d'applications **web et mobiles modernes, performantes et évolutives**.

J'aime apprendre de nouvelles technologies, résoudre des problèmes techniques et transformer des idées en solutions digitales concrètes.

🎯 **Mon objectif :** évoluer en tant qu'ingénieure informatique et contribuer à la réalisation de projets innovants et à forte valeur ajoutée.

---

## 🛠️ Technologies & Compétences

<table align="center">
  <tr>
    <th>💻 Langages</th>
    <th>⚙️ Backend</th>
    <th>🎨 Frontend</th>
  </tr>

  <tr align="center">
    <td>
      <img src="https://skillicons.dev/icons?i=java,php,js" />
    </td>
    <td>
      <img src="https://skillicons.dev/icons?i=laravel,php" />
    </td>
    <td>
      <img src="https://skillicons.dev/icons?i=html,css,bootstrap,tailwind,js" />
    </td>
  </tr>

  <tr>
    <th>📱 Mobile</th>
    <th>🗄️ Bases de données</th>
    <th>🔧 Outils & DevOps</th>
  </tr>

  <tr align="center">
    <td>
      <img src="https://skillicons.dev/icons?i=flutter,androidstudio" />
    </td>
    <td>
      <img src="https://skillicons.dev/icons?i=mysql,mongodb,oracle" />
    </td>
    <td>
      <img src="https://skillicons.dev/icons?i=git,docker,vscode,eclipse,github" />
    </td>
  </tr>

  <tr>
    <th>🧩 Conception & Méthodologies</th>
    <th>🎨 UI/UX & Design</th>
    <th>🖥️ Environnements</th>
  </tr>

  <tr align="center">
    <td>
      UML • POO • Architecture logicielle
    </td>
    <td>
      <img src="https://skillicons.dev/icons?i=figma" />
      <br>
      Canva
    </td>
    <td>
      <img src="https://skillicons.dev/icons?i=windows,linux" />
    </td>
  </tr>
</table>

---

## 📚 Compétences techniques

| Domaine              | Technologies                                         |
| -------------------- | ---------------------------------------------------- |
| **Langages**         | Java • PHP • JavaScript • SQL                        |
| **Backend**          | Laravel • PHP • REST API                             |
| **Frontend**         | HTML5 • CSS3 • JavaScript • Bootstrap • Tailwind CSS |
| **Mobile**           | Flutter • Android • Java/XML                         |
| **Bases de données** | MySQL • MongoDB • Oracle • PL/SQL                    |
| **DevOps**           | Docker • Git • GitHub                                |
| **Conception**       | UML • POO • Architecture logicielle                  |
| **UI/UX**            | Figma • Canva                                        |
| **IDE & Outils**     | VS Code • Android Studio • Eclipse • IntelliJ IDEA   |
| **Environnement**    | Windows • Linux                                      |

---

## 🏆 GitHub Trophies

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=raniajrad&theme=flat&no-frame=true&margin-w=10&row=1" alt="GitHub Trophies" />
</p>

---

## 📊 Statistiques GitHub

<p align="center">
  <img height="180" src="https://github-readme-stats.vercel.app/api?username=raniajrad&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" alt="Statistiques GitHub" />

  <img height="180" src="https://github-readme-streak-stats.herokuapp.com/?user=raniajrad&theme=tokyonight&hide_border=true" alt="GitHub Streak" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=raniajrad&layout=compact&theme=tokyonight&hide_border=true" alt="Langages les plus utilisés" />
</p>

---

## 🚀 Projets

### 👗 Clothing Store — Application E-commerce

<a href="https://github.com/raniajrad/vente-photo">
  <img src="https://img.shields.io/badge/GitHub-Repository-181717?style=for-the-badge&logo=github" />
</a>

Application e-commerce dédiée à la **vente de vêtements**, développée dans le cadre d'un projet de développement web.

**Technologies :**
`Laravel` `PHP` `MySQL` `HTML` `CSS` `JavaScript`

---

### 🏡 Local Platform — Plateforme de services

<a href="https://github.com/raniajrad/platforme-locale">
  <img src="https://img.shields.io/badge/GitHub-Repository-181717?style=for-the-badge&logo=github" />
</a>

Plateforme digitale permettant de mettre en relation des **clients, prestataires de services et administrateurs**.

**Technologies :**
`Laravel` `PHP` `MySQL` `JavaScript` `HTML` `CSS`

---

### 🌐 JS2024 — Projet Front-End

<a href="https://github.com/raniajrad/js2024">
  <img src="https://img.shields.io/badge/GitHub-Repository-181717?style=for-the-badge&logo=github" />
</a>

Projet de formation orienté **développement Front-End**, permettant de mettre en pratique la création d'interfaces web interactives.

**Technologies :**
`HTML5` `CSS3` `JavaScript`

---

## 🎓 Certifications

<p align="center">

<a href="https://www.credly.com/badges/b9693b72-25a6-453e-894f-285eb2fb601c">
  <img src="https://images.credly.com/size/340x340/images/1d37e2c9-e63d-4e64-b758-071509f9469d/image.png" width="130" alt="IBM Spectrum Storage for AI and Big Data Fundamentals"/>
</a>

   

<a href="https://learn.microsoft.com/fr-fr/users/raniajrad-0198/">
  <img src="https://img.shields.io/badge/Microsoft%20Learn-2F2F2F?style=for-the-badge&logo=microsoft&logoColor=white" alt="Microsoft Learn"/>
</a>

</p>

<p align="center">
  <strong>IBM Spectrum Storage for AI and Big Data Fundamentals</strong>
  <br>
  <strong>Microsoft Learn</strong>
</p>

---

## 🌱 Actuellement

🎓 Je poursuis actuellement mon parcours en **cycle ingénieur en informatique**.

📚 Je développe continuellement mes compétences en :

* Développement Full Stack
* Développement mobile
* Architecture logicielle
* Bases de données
* DevOps
* Intelligence artificielle
* Génie logiciel

💡 Je suis également intéressée par les **projets professionnels, stages et opportunités Junior** dans le domaine du développement logiciel.

---

## 💼 Ce que je recherche

Je suis ouverte aux opportunités qui me permettent de :

* 🚀 Participer à des projets réels
* 💻 Développer des applications Web & Mobile
* 🧠 Approfondir mes compétences techniques
* 🤝 Travailler au sein d'une équipe professionnelle
* 📈 Évoluer dans le domaine du génie logiciel

---

## 🤝 Me contacter

<p align="center">

<a href="https://www.linkedin.com/in/rania-jrad/">
  <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>

<a href="mailto:rania.jrad.07@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail"/>
</a>

<a href="https://github.com/raniajrad">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
</a>

</p>

---

<p align="center">
  <strong>✨ Merci de visiter mon profil GitHub ! ✨</strong>
</p>

<p align="center">
  <em>Apprendre • Développer • Innover • Évoluer 🚀</em>
</p>
